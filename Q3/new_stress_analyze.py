...
def read(file):
    ...
    all_content = []  # 存放读取的数据
    for j in range(0, 3):  # 取第0~第2列对的数据
        temp = []
...
answer2 = []
for i in range(0, 3):  # 按照不同的列，根据不同的指标转换为极大型指标，因为只有3列
    answer = None
...
writer = pd.ExcelWriter('New_Analyze.xlsx')  # 写入Excel文件
data.to_excel(writer, 'New_stress', float_format='%.5f')  # ‘page_1’是写入excel的sheet名
writer.save()
writer.close()